...
ZMETA(Gpio)
{
    ...
    ZOBJ_X(ledYellow,"yellow",ZFF_PROP_DEF,"Yellow LED, pin#21");
...
z_status GpioPin::setInput()
{
    ...
    _output=false;
    gGpio.reconfigure_line(_request,_pin,_dir);
    return zs_ok;
}
...
z_status GpioPin::show()
{
    ...
    //if (!_chip)  return zs_not_open;
    gpiod_line_info *info=gpiod_chip_get_line_info(gGpio._chip,_pin);
    if(!info) {
...
{
    if (!_chip)  return zs_not_open;
    if (!request) return zs_bad_parameter;
    if (!initialize())
        return -1;
