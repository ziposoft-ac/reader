...
public:
    ...
    int _id=0;
    int _counter=0;
    TestTimerContext(int id) {

        _id=id;
    }
};
class TestTimerStress : public Test{
public:
    int callback(void*);
    z_status stop() override;
    z_status start() override;
    int _num_timers=10;
    int _delay_min=1000;
    int _delay_max=2000;
    int _step=3;
    int _spawn=2;
    int _max_total_callbacks=100;
    int _callback_count=0;

    z_obj_map_k<int,TestTimerContext,true> _timers;
};
